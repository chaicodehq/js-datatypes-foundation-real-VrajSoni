...
export function updatePrices(menu, increase) {
  ...
  const updatedMenu = Object.fromEntries(
    Object.entries(menu).map(([key,value]) => {return [key, value+increase];})
  );
  return updatedMenu
...
export function mergeDailySpecials(regularMenu, specialsMenu) {
  const superMenu = (typeof regularMenu === "object" && regularMenu !== null && !Array.isArray(regularMenu)) ? regularMenu : {}
  const superSpecialMenu = (typeof specialsMenu === "object" && specialsMenu !== null && !Array.isArray(specialsMenu)) ? specialsMenu : {}
  return {...superMenu,...superSpecialMenu}
}
